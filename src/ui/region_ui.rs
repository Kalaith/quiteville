...
pub fn draw_region_map(
    ...
    for caravan in &trade_manager.caravans {
        if let Some(route) = trade_manager
            .routes
            .iter()
            .find(|r| r.id == caravan.route_id)
        {
...
pub fn draw_node_tooltip(node: &TownNode, mouse_pos: Vec2) {
    ...
    let mut lines = vec![
        ...
        format!(
            "Wood: {:.1}x (base: {:.1}x)",
            node.resource_potentials.wood,
            biome.wood_multiplier()
        ),
        format!(
            "Stone: {:.1}x (base: {:.1}x)",
            node.resource_potentials.stone,
            biome.stone_multiplier()
        ),
        format!(
            "Food: {:.1}x (base: {:.1}x)",
            node.resource_potentials.food,
            biome.food_multiplier()
        ),
        format!(
            "Trade: {:.1}x (base: {:.1}x)",
            node.resource_potentials.trade,
            biome.trade_multiplier()
        ),

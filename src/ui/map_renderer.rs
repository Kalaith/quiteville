...
pub fn draw_map(state: &GameState) {
    ...
    for y in start_y..end_y {
        for x in start_x..end_x {
            if let Some(tile) = map.get_tile(x, y) {
                ...
                // Culling
                if screen_pos.x + size < 0.0
                    || screen_pos.x > screen_width()
                    || screen_pos.y + size < 0.0
                ...
                let final_tex = if is_dormant_zone {
                    "tile_ruins"
                } else {
                    tex_name
                };
    ...
    for (zone_idx, zone) in state.zones.iter().enumerate() {
        ...
        if zone.dormant && !zone.is_under_construction() {
            continue;
        }
            ...
            .find(|t| t.id == zone.template_id)
        {
            if let Some(rect) = template.map_rect {
                ...
                // Culling (Rough)
                if screen_pos.x + width < 0.0
                    || screen_pos.x > screen_width()
                    || screen_pos.y + height < 0.0
                    || screen_pos.y > screen_height()
                {
                    continue;
                }
    ...
    // 6. Draw Particle System (Weather, Smoke)
    state.particle_system.draw(camera);

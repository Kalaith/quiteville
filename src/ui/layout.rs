                        ...
                        if let Some(upgrade_to) = crate::zones::upgrades::can_upgrade(state, idx) {
                                ...
                                state.zone_templates.iter().find(|t| t.id == upgrade_to)
                            {
                                draw_text(
                                    ...
                                    status_y - 45.0,
                                    14.0,
                                    if can_afford { colors::ACCENT } else { GRAY },
                                );
                            }
        ...
        crate::data::Selection::Agent(id) => {
            if let Some(agent) = state.agents.iter().find(|a| a.id == id) {
                ...
                let state_text = match agent.state {
                    ...
                    crate::simulation::agents::AgentState::Socializing { .. } => {
                    ...
                    crate::simulation::agents::AgentState::Building { .. } => {
                        "Building".to_string()
                    }
                };
                draw_text(
